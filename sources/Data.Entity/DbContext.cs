...
namespace UnitOfWork.Data.Entity
{
	public abstract class DbContext : System.Data.Entity.DbContext
	{
		...
		private IEnumerable GetEntitiesByState(EntityState entityState)
			=> (this as IObjectContextAdapter).ObjectContext.ObjectStateManager.GetObjectStateEntries(entityState).Where(entry => !entry.IsRelationship).Select(entry => entry.Entity);
	}
}